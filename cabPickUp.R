...
cabDTL <- cabDTL %>% mutate(pickLat = as.numeric(pickLat), pickLon = as.numeric(pickLon),
             dropLat = as.numeric(dropLat), dropLon = as.numeric(dropLon))

#lat lon check
cabDTL <- cabDTL %>% mutate(llQual = ifelse(((pickLon < 40 & pickLon > 37) & (pickLat > -79 & pickLat < -75)) &
...
LLswap <-LLswap %>% mutate(pickLat = pickLat2) %>%
...
LLT <- LLT %>% mutate(pickupDT=as.POSIXct(pickupDT, format="%Y-%m-%d %H:%M:%S", tz="America/New_York"),
                     dropffDT=as.POSIXct(dropoffDT, format="%Y-%m-%d %H:%M:%S", tz="America/New_York"))

LLT <- LLT %>% mutate(length=difftime(LLT$dropoffDT,LLT$pickupDT,tz="EST"),
                     pHR=hour(pickupDT),
                     dHR=hour(dropoffDT),
                     pDay=wday(pickupDT),
                     dDay=wday(dropoffDT))
...
doll <-cabMap %>% select(dropLon,dropLat,dHR,dDay)
